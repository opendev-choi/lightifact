...
2. 업로드 — 환경변수 `LIGHTIFACT_TOKEN`(사용자가 `lightifact.cardoc.kr/account`에서 발급) 사용, 사내망/VPN 필요:
   ```bash
   curl -s -X POST https://lightifact.cardoc.kr/artifacts \
...
3. **기존 것 수정**(같은 링크 유지, 본인 소유만): 위와 동일하되
   `PUT https://lightifact.cardoc.kr/artifacts/<slug>`.
